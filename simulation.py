...
def create_grid(width, height, randomize=False):
    ...
    if randomize:
        ...
        weights = [16.0, 16.0, 16.0, 16.0, 16.0, 1.0, 16.4049, 0.1, 0.5, 0.3, 1.1, 0.5, 0.5, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.0, 0.01, 0.01, 0.01, 0.01, 0.0001, 0.0001, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.0]
    ...
    for y in range(height):
        for x in range(width):
            ...
            # --- STATE 39: INVESTIGATOR ---
                ...
                random.shuffle(neighbors)
                went_insane = False
                # Check for eldritch horrors
                for ny, nx in neighbors:
                    cy, cx = ny % height, nx % width
                    if grid[cy][cx] in [36, 37, 38]: # Kraken, Cthulhu, Sleeping Cthulhu
                        new_grid[y][x] = 40 # Turn into Cultist
                        went_insane = True
                        break
            ...
            # --- STATE 40: CULTIST ---
                ...
                # 1% chance to summon Cthulhu and sacrifice itself
                summoned = False
                if random.random() < 0.01:
                    for ny, nx in neighbors:
                        cy, cx = ny % height, nx % width
                        if grid[cy][cx] == 6 and new_grid[cy][cx] == 6:
                            new_grid[cy][cx] = 37 # Spawn Cthulhu
                            new_grid[y][x] = 6 # Sacrifice
                            summoned = True
                            break
