...
def print_grid(grid):
    ...
    chars = {0: "R", 1: "P", 2: "S", 3: "K", 4: "L", 5: "B", 6: "V", 7: "*", 8: "@", 9: "W", 10: "G", 11: "J", 12: "M", 13: "X", 14: "A", 15: "Z", 16: "O", 17: "N", 18: "D", 20: "Y", 21: "H", 25: "I", 26: "C", 31: "T", 32: "E", 34: "U", 35: "Q", 36: "^", 44: "F"}
    for row in grid:
    ...
    for y in range(height):
        for x in range(width):
            ...
            # --- STATE 45: YOG-SOTHOTH ---
                ...
                # Transform adjacent basic lifeforms
                neighbors = [(y+dy, x+dx) for dy in [-1, 0, 1] for dx in [-1, 0, 1] if dy != 0 or dx != 0]
                for ny, nx in neighbors:
                    cy, cx = ny % height, nx % width
                    if grid[cy][cx] in [0, 1, 2, 3, 4]:
                        # Corrupt into Cultist or Shoggoth via global modification
                        global_modifications[(cy, cx)] = random.choice([40, 41])

                # Teleport
                if voids:
                    ty, tx = random.choice(voids)
                    global_modifications[(ty, tx)] = 45
                    new_grid[y][x] = 6 # Leave void
                else:
                    new_grid[y][x] = 45 # Stay if no void
                continue
