...
def create_grid(width, height, randomize=False):
    ...
    if randomize:
        ...
        # Weighted choice: RPSLK (80% total, 16% each), Black Hole (1%), Void (16.43%), Supernova (0.1%), Pulsar (0.5%), Wormhole (0.3%), Godzilla (1.1%), Jaeger (0.5%), Mothra (0.5%), Glitch (0.05%), Anti-Virus (0.05%), MechaGodzilla (0.05%), Omega (0.05%), Nexus (0.05%), Reaper (0.05%), Phoenix (0.05%), Yggdrasil (0%), Nidhogg (0.01%), Pandora (0.01%), Chronos (0.01%), Paradox (0.01%), Singularity (0.0001%), Conway (0.0001%), Neutron Star Ortho (0.005%), Neutron Star Diag (0.005%), Radiotroph (0.005%), Black Monolith (0.005%), Tardigrade (0.005%), White Hole (0.005%), Leviathan (0.005%), Ahab (0.005%), Moby Dick (0.005%), Kraken (0.005%), Cthulhu (0.005%), Sleeping Cthulhu (0.0%)
        weights = [16.0, 16.0, 16.0, 16.0, 16.0, 1.0, 16.4099, 0.1, 0.5, 0.3, 1.1, 0.5, 0.5, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.0, 0.01, 0.01, 0.01, 0.01, 0.0001, 0.0001, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.005, 0.0]
...
def print_grid(grid):
    ...
    chars = {0: "R", 1: "P", 2: "S", 3: "K", 4: "L", 5: "B", 6: "V", 7: "*", 8: "@", 9: "W", 10: "G", 11: "J", 12: "M", 13: "X", 14: "A", 15: "Z", 16: "O", 17: "N", 18: "D", 20: "Y", 21: "H", 25: "I", 26: "C", 31: "T", 32: "E", 34: "U", 35: "Q", 36: "^", 37: "C", 38: "S"}
    for row in grid:
        ...
        for x in range(width):
            ...
            # --- STATE 37: CTHULHU ---
                ...
                for ny, nx in neighbors:
                    cy, cx = ny % height, nx % width
                    if grid[cy][cx] == 36:
                        new_grid[cy][cx] = 37 # Consume Kraken, spawn new Cthulhu
                        consumed_kraken = True
                        break

                if consumed_kraken:
                    # Spawn another Cthulhu in an adjacent void if possible
                    spawned = False
                    for ny, nx in neighbors:
                        cy, cx = ny % height, nx % width
                        if grid[cy][cx] == 6 and new_grid[cy][cx] == 6:
                            ...
                            spawned = True
                            break
                    new_grid[y][x] = 37 # Stay in place
                else:
                    if random.random() < 0.01:
                        new_grid[y][x] = 38 # 1% chance to fall asleep (State 38)
                    else:
                        # Wander into a void (10% chance)
                        if random.random() < 0.10:
                            moved = False
            ...
            # --- STATE 38: SLEEPING CTHULHU ---
                ...
                if random.random() < 0.01:
                    new_grid[y][x] = 37 # 1% chance to awaken
                else:
